#!/usr/bin/env python3
"""
Web Interface Server for NDI Receiver
Starts HTTP server to serve the web interface and handle WebSocket connections
"""
...
def start_server(port=80, websocket_port=8080, auto_open=True):
    ...
    print()
    print(f"Make sure NDI Receiver is running with --web-server flag")
    print("=" * 60)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python start_server.py                           # Default ports: HTTP=8000, WebSocket=8080
  ...
  sudo python start_server.py --port 80           # Use standard HTTP port (requires sudo)
        """
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=8000,
        help='Port number for the HTTP server (default: 8000, use 80 for standard but requires sudo)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=8080,
        help='Port number for WebSocket connection (default: 8080)'
    )
...
def main():
    ...
    print("=" * 60)
    print("NDI Receiver Web Interface")
    ...
    print(f"WebSocket server runs on port {args.websocket_port}")
    print(f"Make sure NDI Receiver is running with --web-server flag")
    print("=" * 60)
