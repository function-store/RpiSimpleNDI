...
def start_server(port=8000, websocket_port=8080, auto_open=True):
...
async def handle_td_websocket(websocket):
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python bridge_server.py                          # Default: HTTP=8000, Browser WS=8080, TD=8081
  python bridge_server.py --port 8090             # Custom HTTP port
  python bridge_server.py -w 9000 -t 9001        # Custom WebSocket ports
  python bridge_server.py --no-browser            # Don't open browser automatically
...
TouchDesigner Setup:
  WebSocket DAT connects as a CLIENT to the bridge server:
    ...
    parser.add_argument(
        ...
        type=int,
        default=8000,
        help='Port number for the HTTP server (default: 8000)'
    )
